...
export function StockDetailPage({ stock: initialStock, onBack, onNavigateToLearning }: StockDetailPageProps) {
  ...
  const USD_TO_KRW = 1320; // 달러-원화 환율
  ...
  useEffect(() => {
    const generateOrderBook = () => {
      ...
      for (let i = 5; i >= 1; i--) {
        const priceIncrement = stock.type === 'domestic' ? 100 * i : 0.1 * i;
        asks.push({
      ...
      for (let i = 1; i <= 5; i++) {
        const priceDecrement = stock.type === 'domestic' ? 100 * i : 0.1 * i;
        bids.push({
          price: basePrice - priceDecrement,
          volume: Math.floor(Math.random() * 5000) + 1000
        });
      }
  ...
  }, [stock.price, stock.type]);

  // 차트 데이터 생성 (타임프레임 변경시에만) - 캔들스틱 형태로
  useEffect(() => {
    const generateChartData = () => {
      const data = [];
      const basePrice = initialStock.price;
      const basePoints = timeFrame.includes('min') ? 60 : timeFrame === 'day' ? 30 : timeFrame === 'week' ? 7 : timeFrame === 'month' ? 30 : 12;
      const points = Math.floor(basePoints * zoomLevel);
      ...
      for (let i = 0; i < points; i++) {
        ...
        // 종가 계산
        const changePercent = (Math.random() - 0.5) * 3; // -1.5% ~ +1.5%
        ...
        // 소수점 처리
        const decimals = initialStock.type === 'domestic' ? 0 : 2;
        const openFixed = parseFloat(open.toFixed(decimals));
        const highFixed = parseFloat(high.toFixed(decimals));
        const lowFixed = parseFloat(low.toFixed(decimals));
        const closeFixed = parseFloat(close.toFixed(decimals));
        ...
        data.push({
          ...
          low: lowFixed,
          close: closeFixed,
          color: closeFixed >= openFixed ? '#ef4444' : '#3b82f6' // 양봉: 빨강, 음봉: 파랑
        });
        
        // 다음 캔들의 시가 = 현재 캔들의 종가
        currentPrice = closeFixed;
      }
      return data;
    };

    setChartData(generateChartData());
  }, [timeFrame, initialStock.price, initialStock.type, initialStock.code, zoomLevel]);

  // 실시간 가격 업데이트
  useEffect(() => {
    const interval = setInterval(() => {
      setStock(prevStock => {
        const priceChangePercent = (Math.random() - 0.5) * 0.5;
        const priceChange = prevStock.price * (priceChangePercent / 100);
        const newPrice = Math.max(prevStock.price + priceChange, prevStock.price * 0.5);
        
        const changeRateAdjust = (Math.random() - 0.5) * 0.2;
        const newChangeRate = prevStock.changeRate + changeRateAdjust;
        
        return {
          ...prevStock,
          price: parseFloat(newPrice.toFixed(prevStock.type === 'domestic' ? 0 : 2)),
  ...
  const handlePercentageSelect = (percentage: number) => {
    ...
    if (orderType === 'buy') {
      ...
      const availableFunds = stock.type === 'foreign' ? balance / USD_TO_KRW : balance;
  ...
  const handleConfirmOrder = () => {
    ...
    if (pendingAction.type === 'buy') {
      const totalInKRW = stock.type === 'foreign' ? total * USD_TO_KRW : total;
      ...
      setBalance(prev => prev + (stock.type === 'foreign' ? total * USD_TO_KRW : total));
    }
            ...
            <div className="text-right">
              ...
              {stock.type === 'foreign' && (
                <div className="text-sm text-gray-600 mb-1">
                  ≈ ₩{formatNumber(Math.floor(stock.price * USD_TO_KRW))}
                </div>
              )}
              <div
                ...
                {stock.type === 'foreign' ? (
                  <>
                    <div className="text-gray-900 font-bold">
                      ${formatNumber(Math.floor(balance / USD_TO_KRW * 100) / 100)}
                    </div>
            ...
            <div>
              ...
              {stock.type === 'foreign' && orderPrice && (
                <div className="text-xs text-gray-600 mt-1">
                  ≈ ₩{formatNumber(Math.floor(parseFloat(orderPrice) * USD_TO_KRW))}
                </div>
              )}
              {priceType !== 'market' && (
                          ...
                          <div className="text-right">
                            ...
                            {item.type === 'foreign' && (
                              <div className="text-xs text-gray-400">
                                ≈ ₩{formatNumber(Math.floor(item.currentPrice * USD_TO_KRW))}
                              </div>
                            )}
                          </div>
                        </div>
