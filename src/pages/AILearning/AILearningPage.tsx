...
const tabItems: Array<{ key: AILearningTab; label: string; icon: string }> = [
  { key: "curriculum", label: "학습 과정", icon: "📚" },
  { key: "profile", label: "나의 투자 성향", icon: "👤" },
  { key: "tutor", label: "AI 튜터", icon: "🤖" },
];
...
const AILearningPage = () => {
  ...
  const [isRetakingType, setIsRetakingType] = useState(false);
  ...
  const confirmType = () => {
    ...
    localStorage.setItem("investmentTypeSource", "quiz");
    setSelectedType(draftType);
    setActiveTab("curriculum");
    setIsRetakingType(false);
  };
          ...
          <div className="mx-auto max-w-7xl px-6 py-6">
            <div className="flex items-center gap-4">
              <div className="rounded-2xl bg-gradient-to-br from-blue-500 to-blue-600 p-3 shadow-lg">
                <span className="flex h-8 w-8 items-center justify-center text-sm font-bold text-white">AI</span>
              </div>
              <div>
                <h1 className="text-3xl font-bold text-[#1D1E20]">투자 성향 진단</h1>
                <p className="mt-1 text-sm text-[#A5A6A9]">로그인 후 처음 한 번 진행하는 AI 대화형 투자 성향 체크</p>
              </div>
            </div>
          </div>
        </section>
                ...
                <div className={`rounded-2xl border-2 p-8 shadow-sm ${investmentTypes[selectedType].bg} ${investmentTypes[selectedType].border}`}>
                  <div className="mb-6 flex items-center gap-6">
                    ...
                    <button
                      ...
                      className="shrink-0 rounded-xl border border-[#42D6BA]/40 bg-white px-4 py-2 text-sm font-semibold text-[#3AB8A8] transition-colors hover:bg-[#C7F3EB]/20"
                    >
                      성향 다시 진단
                    </button>
                  </div>
