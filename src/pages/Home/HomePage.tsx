...
function buildNaverNewsSearchUrl(query: string): string {
...
const HomePage: React.FC = () => {
  ...
  const selectedCategoryName = useMemo(
    () => visibleCategories.find((category) => category.categoryId === selectedCategoryId)?.categoryName ?? "",
            ...
            <div className="flex flex-col gap-3">
              {latestNewsLoading ? (
                ...
                latestNews.map((news) => {
                  ...
                  const naverNewsUrl = buildNaverNewsSearchUrl(
                    [selectedCategoryName, news.title].filter(Boolean).join(" ")
                  );
                  return (
                    <RelatedNews
